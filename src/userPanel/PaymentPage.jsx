...
//   console.log(courseId,userId);
  ...
  return (
    <div className="container py-4">
      <button className="btn btn-outline-info mb-3" onClick={() => navigate(-1)}>
        ← Back
      </button>
      ...
      <div className="row">
        ...
        <div className="col-lg-6 mb-4">
          <div className="card p-3 shadow-sm">
            ...
            <p><strong>Price:</strong> <span className="text-success fw-bold">₹{course.price}</span></p>
          ...
          <div className="d-flex flex-wrap">
            {lessons.map((l, i) => (
              <div key={l.id || i} className="m-2 position-relative"
                   style={{width:250,height:150,backgroundImage:`url(${course.thumbnail})`,
                           backgroundSize:"cover",borderRadius:10}}>
                <div className="position-absolute top-50 start-50 translate-middle">
                  <PlayCircle size={40} color="white" />
                </div>
                <div className="position-absolute bottom-0 w-100 bg-dark bg-opacity-50 text-white p-1 text-center">
                  {l.lesson_title}
                </div>
              </div>
            ))}
          ...
          <div className="card mt-4 p-3 shadow-sm">
            ...
            {comments.length === 0 ? (
              ...
              <div key={c.id || i} className="border rounded p-2 mb-2 bg-light">
        ...
        <div className="col-lg-6">
          ...
          {showForm === "card" && (
            <form className="border rounded p-3 shadow-sm" onSubmit={handlePay}>
              <input
                className="form-control mb-2"
                placeholder="Card Number"
                maxLength={19}
                value={form.number}
                onChange={(e) => setForm({...form, number: e.target.value.replace(/\D/g, "").replace(/(.{4})/g, "$1-").slice(0,19)})}
              />
              ...
              <button className="btn btn-success w-100 mt-2" type="submit">Pay Now</button>
            </form>
          )}

          <div
            className="list-group-item p-3 mt-3 border rounded shadow-sm"
          ...
          {showForm === "upi" && (
            <div className="border rounded p-3 shadow-sm text-center">
              <p className="text-primary fw-semibold">UPI function not active, use Card instead.</p>
